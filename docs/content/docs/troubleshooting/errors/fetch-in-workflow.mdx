...
import { generateText } from 'ai';
import { openai } from '@ai-sdk/openai';
import { fetch } from '@vercel/workflow'; // [!code highlight]
...
For more details on the `fetch` step function, see the [fetch API reference](/docs/api-reference/vercel-workflow-core/fetch).
