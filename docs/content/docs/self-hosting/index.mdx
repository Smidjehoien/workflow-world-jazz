...
Workflows can run on any infrastructure through _**Worlds**_. A World is an adapter responsible for handling workflow storage, queuing, authentication, and streaming through a given backend.
...
The Workflow SDK ships with two world implementations:

<Cards>
  <Card title="Embedded World" href="/docs/api-reference/world/world-embedded">
    Filesystem-based backend for local development. Stores data in `.workflow-data/` directory.
  </Card>
  <Card title="Vercel World" href="/docs/api-reference/world/world-vercel">
...
On top of the default Worlds provided by Workflow SDK, you can also build a new world implementation for custom infrastructure:
...
To build a custom world, use a community-implemented `World`, or implement the `World` interface yourself, which combines:
...
See the [World API Reference](/docs/api-reference/world) for implementation details.
...
npx wf i runs --backend <your-world-name>
```
...
- [Embedded World](/docs/api-reference/world/world-embedded) - Local development backend
- [Vercel World](/docs/api-reference/world/world-vercel) - Production backend for Vercel
- [World API Reference](/docs/api-reference/world) - Building custom worlds
