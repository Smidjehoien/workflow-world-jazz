...
The **Embedded World** (`@vercel/workflow-world-embedded`) is a filesystem-based workflow backend designed for local development and testing. It stores workflow data as JSON files on disk and provides in-memory queuing.

## Overview

The embedded world is perfect for local development because it:
...
The embedded world stores all workflow data as JSON files in a configurable directory:
...
The embedded world uses an in-memory queue with HTTP transport:
...
The embedded world provides a simple authentication implementation:
...
Since it's for local development, no authentication is required or enforced.

## Configuration

### Data Directory

By default, workflow data is stored in `.workflow-data/` in your project root. You can customize this:
...
The embedded world automatically detects your server port from the `PORT` environment variable:
...
The embedded world is used automatically during local development:

```bash
# Start your Next.js dev server
npm run dev

# Workflows automatically use embedded world
```

### Manual

You can explicitly set the embedded world:
...
A typical development workflow with embedded world:
...
3. **Inspect the results:**
   - Use the [CLI](/docs/api-reference/cli) or [Web UI](/docs/observability)
...
The embedded world integrates with the Workflow SDK's observability tools:
...
The embedded world is designed for development, not production:
...
For production deployments, use the [Vercel World](/docs/api-reference/world/world-vercel).

## API Reference

### createEmbeddedWorld

Creates an embedded world instance:
...
- [World Interface](/docs/api-reference/world) - Understanding the World interface
- [Vercel World](/docs/api-reference/world/world-vercel) - For production deployments
