...
- Throwing [`FatalError`](/docs/api-reference/vercel-workflow-core/fatal-error) for client errors (400-499) to prevent retries
- Handling 429 rate limiting by reading the `Retry-After` header and using [`RetryableError`](/docs/api-reference/vercel-workflow-core/retryable-error)
