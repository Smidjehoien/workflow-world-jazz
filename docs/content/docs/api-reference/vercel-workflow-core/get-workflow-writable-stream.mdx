---
title: getWorkflowWritableStream
---

# `getWorkflowWritableStream()`

Retrieves the current workflow run's default writable stream. The writable stream is intended to be passed as an argument to steps which can write to it. Chunks written to this stream can be read outside the workflow by using [`getWorkflowReadableStream`](/docs/api-reference/get-workflow-readable-stream).

You should use this in your workflows to produce streaming output that can be consumed by clients in real-time.
...
The `getWorkflowWritableStream` function takes no parameters.
...
import { sleep, getWorkflowWritableStream } from '@vercel/workflow-core';

export async function outputStreamWorkflow() {
  "use workflow";

  const writable = getWorkflowWritableStream(); // [!code highlight]
...
async function stepWithOutputStream(writable: WritableStream) {
  ...
  await writer.write(new TextEncoder().encode('Hello, world!')); // [!code highlight]
  writer.releaseLock();
}
...
async function stepCloseOutputStream(writable: WritableStream) {
  ...
  await writable.close(); // [!code highlight]
}
```
...
export async function chat(messages: UIMessage[]) {
  ...
  const writable = getWorkflowWritableStream<UIMessageChunk>(); // [!code highlight]
...
async function streamTextStep(writable: WritableStream<UIMessageChunk>) {
  ...
  while (true) {
    ...
    if (done) break;
    await writer.write(value); // [!code highlight]
  }
...
To consume the data written to this stream from outside the workflow, use the corresponding [`getWorkflowReadableStream`](/docs/api-reference/get-workflow-readable-stream) function with the workflow run ID.
