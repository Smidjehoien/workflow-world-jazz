...
Retrieves the current workflow run's default writable stream. The writable stream is intended to be passed as an argument to steps which can write to it. Chunks written to this stream can be read outside the workflow by using [`getWorkflowRun()`](/docs/api-reference/vercel-workflow-core/get-workflow-run).
...
```typescript lineNumbers
import { sleep, getWorkflowWritableStream } from '@vercel/workflow';
