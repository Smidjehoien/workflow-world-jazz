...
Retrieves the current workflow run's default writable stream. The writable stream is intended to be passed as an argument to steps which can write to it. Chunks written to this stream can be read outside the workflow by using [`getWorkflowReadableStream`](/docs/api-reference/vercel-workflow-core/get-workflow-readable-stream).
...
definition={generateDefinition({
  code: `
import { getWorkflowWritableStream } from '@vercel/workflow-core';
export default getWorkflowWritableStream;`
})}
...
To consume the data written to this stream from outside the workflow, use the corresponding [`getWorkflowReadableStream`](/docs/api-reference/vercel-workflow-core/get-workflow-readable-stream) function with the workflow run ID.
