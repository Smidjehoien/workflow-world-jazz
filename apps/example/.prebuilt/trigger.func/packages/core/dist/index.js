...
import { InvalidCallbackError, parseCallbackRequest, QueueClient, send, Topic, } from '@vercel/queue';
 ...
 */
export async function start(workflowId, options = {}) {
 ...
 */
export function vercelAPIWorkflowsEntrypoint(workflowCode) {
    return async (request) => {
        try {
            ...
            if (consumerGroup !== 'default') {
                throw new Error(`Expected 'default' consumer group, got '${consumerGroup}'`);
            }
            ...
            if (error instanceof InvalidCallbackError) {
                return Response.json({ error: 'Invalid callback request' }, { status: 400 });
            }
            return Response.json({ error: 'Failed to process callback' }, { status: 500 });
        }
    };
}
function workflowMessageHandler(workflowCode, workflowName) {
    return async (message_, metadata) => {
        ...
        try {
            ...
            if (isInstanceOf(err, StepNotRunError)) {
                ...
                const stepInvokePayload = {
                    ...message,
                    stepId: err.stepId,
                    arguments: err.args,
                };
                const callbackUrl = new URL(message.callbackUrl);
                const stepCallbackUrl = new URL(`/api/generated/steps${callbackUrl.search}`, callbackUrl);
                ...
                console.error(`Workflow failed with a fatal error (Run ID: ${message.runId}): ${err}`);
            }
            else {
                console.error(`Unexpected error while running workflow (Run ID: ${message.runId}): ${err}`);
            }
        }
    };
}
 ...
 */
export async function vercelAPIStepsEntrypoint(request) {
    try {
        // Parse the queue callback information
        const { queueName, consumerGroup, messageId } = parseCallbackRequest(request);
...
function stepMessageHandler(stepFn) {
    return async (message_, metadata) => {
        // TODO: validate `message` schema
        const message = message_;
        ...
        try {
            ...
            console.error(`${getErrorName(err)} while running "${message.stepId}" step (Workflow run ID: ${message.runId}): ${String(err)}`);
