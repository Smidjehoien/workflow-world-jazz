...
describe('Webhook route generation', () => {
  ...
  describe('Build Output API build target', () => {
    it('should create webhook function at correct location', async () => {
      ...
      // Don't actually bundle in tests since it requires @vercel/workflow to be installed
      ...
      expect(funcContent).toContain('@vercel/workflow/api');
    ...
    it('should create correct .vc-config.json', async () => {
      ...
      await buildWebhookFunction({
        inputFiles: [],
        workflowGeneratedDir,
        bundle: false, // Skip bundling in tests (requires @vercel/workflow installed)
      });
