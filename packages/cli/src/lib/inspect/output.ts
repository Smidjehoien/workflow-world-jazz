...
const formatIdField = (
  ...
  let idStr = value instanceof StreamID ? value.toString() : String(value);
  idStr = shouldTruncateIds ? truncateIdToLastChars(idStr) : idStr;
  if (prop === 'streamId') {
...
const showInspectInfoBox = (resource: string) => {
  logger.info(
    `To view details for a ${resource}, use \`wf i ${resource}\` <id>`
  );
  logger.info(
