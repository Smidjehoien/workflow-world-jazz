...
const validBuildTargets = ['vercel-static', 'vercel-build-output-api'];
function isValidBuildTarget(target) {
    return validBuildTargets.includes(target);
}
const defaultBuildTarget = 'vercel-static';
...
if (cliOptions.buildTarget && !isValidBuildTarget(cliOptions.buildTarget)) {
    console.warn(`Warning: Invalid target "${cliOptions.buildTarget}". Using default "${defaultBuildTarget}".`);
...
const stepBundleEntrypoint = join(__dirname, 'entrypoints', 'vercelAPIStepEntrypoint.js');
const inputEntrypoints = {
    include: resolvedConfig.dirs.map((dir) => `${resolve(resolvedConfig.workingDir, dir)}/**/*.{ts,tsx,mts,cts,js,jsx,mjs,cjs}`),
    exclude: [
        '**/node_modules/**',
        '**/.git/**',
        '**/.next/**',
...
async function buildVercelStatic() {
    // Steps Bundle
    console.log('Creating Vercel API steps bundle at', resolvedConfig.stepsBundlePath);
    const stepBundle = await rollup({
    ...
    // Workflows Bundle
    console.log('Creating vercel API workflows bundle at', resolvedConfig.workflowBundle);
    const workflowsBundle = await rollup({
        // @ts-expect-error - multi plugin changes the input type
    ...
    const workflowBundlePath = resolve(resolvedConfig.workingDir, resolvedConfig.workflowBundle);
    await writeFile(workflowBundlePath, `import { vercelAPIWorkflowsEntrypoint } from '@vercel/workflow-core';
export const POST = vercelAPIWorkflowsEntrypoint(
  ${JSON.stringify(workflowBundleCode)}
);`);
}
async function buildVercelBuildOutputAPI() {
    const outputDir = resolve(resolvedConfig.workingDir, resolvedConfig.buildOutputDir);
    ...
    const embeddedCodeBundle = await rollup({
        // @ts-expect-error - multi plugin changes the input type
        input: inputEntrypoints,
        treeshake: 'smallest',
        plugins: [
...
export default function handler(req, res) {
  ...
  return entrypoint(req, res);
    ...
    const workflowBundle = await rollup({
        input: 'entry',
        treeshake: 'smallest',
        plugins: [
            // @ts-expect-error - default export is a function
