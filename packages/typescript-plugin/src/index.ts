  ...
  function create(info: ts.server.PluginCreateInfo) {
    try {
      ...
      info.project.projectService.logger.info(
        '@vercel/workflow-typescript-plugin: Initializing plugin'
      );
      ...
      info.project.projectService.logger.info(
        `@vercel/workflow-typescript-plugin: Diagnostics=${enableDiagnostics}, Completions=${enableCompletions}`
      );
      ...
      if (enableDiagnostics) {
        proxy.getSemanticDiagnostics = (fileName: string) => {
          ...
          try {
            ...
            info.project.projectService.logger.info(
              `@vercel/workflow-typescript-plugin: Error in getSemanticDiagnostics: ${error}`
            );
            return prior;
          }
        };
      ...
      if (enableCompletions) {
        proxy.getCompletionsAtPosition = (
          ...
          try {
            ...
            info.project.projectService.logger.info(
              `@vercel/workflow-typescript-plugin: Error in getCompletionsAtPosition: ${error}`
            );
            return prior;
          }
        };
      ...
      info.project.projectService.logger.info(
        '@vercel/workflow-typescript-plugin loaded successfully'
      );

      return proxy;
    } catch (error) {
      info.project.projectService.logger.info(
        `@vercel/workflow-typescript-plugin: Error initializing plugin: ${error}`
      );
